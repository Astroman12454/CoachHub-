...
interface ExerciseCommentsDialogProps {
  ...
  onCommentCountChange?: (exerciseId: number, delta: number) => void;
}

// Opened from the comment icon on a community exercise card — a lightweight
// thread scoped to one exercise, not a full page. Posting requires a public
// name (same 409 PUBLIC_NAME_REQUIRED gate as publishing/liking a coach's
// identity onto visible content) — the dialog stays open and hands off to
// SetPublicNameDialog, then retries automatically on success, same pattern
// as ExerciseLibrary's publish flow.
export default function ExerciseCommentsDialog({ exerciseId, exerciseName, onOpenChange, onCommentCountChange }: ExerciseCommentsDialogProps) {
  ...
  const postCommentMutation = useMutation({
    mutationFn: async (body: string) => {
      const res = await apiRequest("POST", `/api/community-exercises/${exerciseId}/comments`, { body });
      return res.json() as Promise<ExerciseComment>;
    },
    onSuccess: (comment) => {
      queryClient.setQueryData<ExerciseComment[]>(queryKey, (old = []) => [...old, comment]);
      if (exerciseId !== null) onCommentCountChange?.(exerciseId, 1);
      setDraft("");
    },
    onError: (error) => {
  ...
  const deleteCommentMutation = useMutation({
    mutationFn: async (commentId: number) => apiRequest("DELETE", `/api/community-exercises/${exerciseId}/comments/${commentId}`),
    onMutate: async (commentId) => {
      const previous = queryClient.getQueryData<ExerciseComment[]>(queryKey);
      queryClient.setQueryData<ExerciseComment[]>(queryKey, (old = []) => old.filter((c) => c.id !== commentId));
      return { previous };
    },
    onSuccess: () => {
      if (exerciseId !== null) onCommentCountChange?.(exerciseId, -1);
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>{t("exerciseCommentsDialog.description", { name: exerciseName })}</DialogDescription>
          </DialogHeader>
