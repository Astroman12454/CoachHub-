...
interface PhysicalTestCommentsDialogProps {
  testId: number | null;
  testName: string;
...
// Physical-test twin of ExerciseCommentsDialog/PlayCommentsDialog — same
// behavior (public name required to post, author or test owner can
// delete), pointed at the community-physical-tests endpoints.
export default function PhysicalTestCommentsDialog({ testId, testName, onOpenChange, onCommentCountChange }: PhysicalTestCommentsDialogProps) {
  ...
  const queryKey = [`/api/community-physical-tests/${testId}/comments`];
  const { data: comments = [], isLoading } = useQuery<PhysicalTestComment[]>({
    queryKey,
    enabled: open,
  });
  ...
  const postCommentMutation = useMutation({
    mutationFn: async (body: string) => {
      const res = await apiRequest("POST", `/api/community-physical-tests/${testId}/comments`, { body });
      return res.json() as Promise<PhysicalTestComment>;
    },
    onSuccess: (comment) => {
      queryClient.setQueryData<PhysicalTestComment[]>(queryKey, (old = []) => [...old, comment]);
  ...
  const deleteCommentMutation = useMutation({
    mutationFn: async (commentId: number) => apiRequest("DELETE", `/api/community-physical-tests/${testId}/comments/${commentId}`),
    onMutate: async (commentId) => {
      const previous = queryClient.getQueryData<PhysicalTestComment[]>(queryKey);
      queryClient.setQueryData<PhysicalTestComment[]>(queryKey, (old = []) => old.filter((c) => c.id !== commentId));
      return { previous };
    },
    onSuccess: () => {
      if (testId !== null) onCommentCountChange?.(testId, -1);
