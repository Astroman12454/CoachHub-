...
// Publishes a fresh evaluation test to the community under the given agent,
// setting a public name first since that's required to publish. No plan
// gate applies — creating/importing evaluation tests is free on every plan.
async function shareNewTest(agent: request.Agent, overrides: Partial<Record<string, unknown>> = {}) {
  ...
  const create = await agent.post("/api/evaluation-tests").send(testBody(overrides));
  await agent.put(`/api/evaluation-tests/${create.body.id}/share-community`).send({ shared: true });
  return create.body.id as number;
}

describe("evaluation test community sharing", () => {
  ...
  it("toggles community sharing, requiring a public name first", async () => {
    ...
    const create = await agent.post("/api/evaluation-tests").send(testBody());

    const shareWithoutName = await agent.put(`/api/evaluation-tests/${create.body.id}/share-community`).send({ shared: true });
    expect(shareWithoutName.status).toBe(409);
    expect(shareWithoutName.body.code).toBe("PUBLIC_NAME_REQUIRED");

    await agent.put("/api/account/public-name").send({ publicName: "Coach Fitness" });
    const share = await agent.put(`/api/evaluation-tests/${create.body.id}/share-community`).send({ shared: true });
    expect(share.status).toBe(200);
    expect(share.body.sharedToCommunity).toBe(1);

    const unshare = await agent.put(`/api/evaluation-tests/${create.body.id}/share-community`).send({ shared: false });
  ...
  it("scopes sharing to the owning account — an outsider gets 404", async () => {
    ...
    const create = await owner.post("/api/evaluation-tests").send(testBody());

    const { agent: outsider } = await signedInAgent(app);
    const res = await outsider.put(`/api/evaluation-tests/${create.body.id}/share-community`).send({ shared: true });
  ...
  it("lists shared tests with publishedBy resolved to the owner's public name", async () => {
    ...
    const res = await viewer.get("/api/community-evaluation-tests");
  ...
  it("imports a community test as a fresh copy — free on every plan, no capacity limit", async () => {
    ...
    const imported = await importer.post(`/api/community-evaluation-tests/${testId}/import`);
    ...
    const inLibrary = await importer.get("/api/evaluation-tests");
  ...
  it("404s importing a test that isn't shared or doesn't exist", async () => {
    const { agent: owner } = await signedInAgent(app);
    const notShared = await owner.post("/api/evaluation-tests").send(testBody());

    const { agent: importer } = await signedInAgent(app);
    const res = await importer.post(`/api/community-evaluation-tests/${notShared.body.id}/import`);
    expect(res.status).toBe(404);

    const unknown = await importer.post("/api/community-evaluation-tests/999999/import");
    expect(unknown.status).toBe(404);
  });
});

describe("evaluation test community likes", () => {
  let app: express.Express;
  ...
  it("likes and unlikes a shared test, toggling likeCount and likedByMe", async () => {
    ...
    const like = await liker.post(`/api/community-evaluation-tests/${testId}/like`);
    expect(like.status).toBe(204);

    const afterLike = await liker.get("/api/community-evaluation-tests");
    expect(afterLike.body.find((t: { id: number }) => t.id === testId)).toMatchObject({ likeCount: 1, likedByMe: true });

    const unlike = await liker.delete(`/api/community-evaluation-tests/${testId}/like`);
    expect(unlike.status).toBe(204);

    const afterUnlike = await liker.get("/api/community-evaluation-tests");
  ...
  it("404s liking a test that isn't shared or doesn't exist", async () => {
    ...
    const { agent: liker } = await signedInAgent(app);
    const res = await liker.post(`/api/community-evaluation-tests/${notShared.body.id}/like`);
    expect(res.status).toBe(404);
  });
  ...
  it("notifies the test's owner on a new like, never for liking your own", async () => {
    ...
    await liker.post(`/api/community-evaluation-tests/${testId}/like`);

    const notifications = await owner.get("/api/notifications");
    expect(notifications.body.notifications[0]).toMatchObject({ type: "like_evaluation_test", actorPublicName: "Test Liker", evaluationTestName: "Notify Like Test" });

    await owner.post(`/api/community-evaluation-tests/${testId}/like`);
...
describe("evaluation test community saves (bookmarks)", () => {
  let app: express.Express;
  ...
  it("saves and unsaves a shared test, separate from liking", async () => {
    ...
    const save = await saver.post(`/api/community-evaluation-tests/${testId}/save`);
    expect(save.status).toBe(204);

    const afterSave = await saver.get("/api/community-evaluation-tests");
    const found = afterSave.body.find((t: { id: number }) => t.id === testId);
    expect(found.savedByMe).toBe(true);
    expect(found.likedByMe).toBe(false);

    const unsave = await saver.delete(`/api/community-evaluation-tests/${testId}/save`);
    expect(unsave.status).toBe(204);

    const afterUnsave = await saver.get("/api/community-evaluation-tests");
  ...
  it("filters the community feed to only saved tests with ?saved=true", async () => {
    ...
    const { agent: saver } = await signedInAgent(app);
    await saver.post(`/api/community-evaluation-tests/${savedId}/save`);

    const res = await saver.get("/api/community-evaluation-tests?saved=true");
  ...
  it("404s saving a test that isn't shared or doesn't exist", async () => {
    ...
    const { agent: saver } = await signedInAgent(app);
    const res = await saver.post(`/api/community-evaluation-tests/${notShared.body.id}/save`);
    expect(res.status).toBe(404);
  });
});

describe("evaluation test comments", () => {
  let app: express.Express;
  ...
  it("posts a comment and returns it with the author's public name", async () => {
    ...
    const res = await commenter.post(`/api/community-evaluation-tests/${testId}/comments`).send({ body: "Good test!" });
  ...
  it("requires a public name before commenting", async () => {
    ...
    const { agent: commenter } = await signedInAgent(app);
    const res = await commenter.post(`/api/community-evaluation-tests/${testId}/comments`).send({ body: "Nice!" });
  ...
  it("lets the comment's author or the test's owner delete a comment, nobody else", async () => {
    ...
    const create = await commenter.post(`/api/community-evaluation-tests/${testId}/comments`).send({ body: "Rude comment" });

    const { agent: outsider } = await signedInAgent(app);
    const deniedDelete = await outsider.delete(`/api/community-evaluation-tests/${testId}/comments/${create.body.id}`);
    expect(deniedDelete.status).toBe(404);

    const ownerDelete = await owner.delete(`/api/community-evaluation-tests/${testId}/comments/${create.body.id}`);
  ...
  it("notifies the test's owner on a new comment, never for commenting on your own", async () => {
    ...
    await commenter.post(`/api/community-evaluation-tests/${testId}/comments`).send({ body: "Nice" });

    const notifications = await owner.get("/api/notifications");
    expect(notifications.body.notifications[0]).toMatchObject({ type: "comment_evaluation_test", actorPublicName: "Notify Test Commenter" });

    await owner.post(`/api/community-evaluation-tests/${testId}/comments`).send({ body: "Thanks, self-comment" });
  ...
  it("reflects commentCount in the community evaluation tests list", async () => {
    const { agent: owner } = await signedInAgent(app);
    ...
    await commenter.post(`/api/community-evaluation-tests/${testId}/comments`).send({ body: "One" });
    await commenter.post(`/api/community-evaluation-tests/${testId}/comments`).send({ body: "Two" });

    const res = await owner.get("/api/community-evaluation-tests");
